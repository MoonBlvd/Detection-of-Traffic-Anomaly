...
class DistributedWeightedSampler(Sampler):
    ...
    def __iter__(self):
        if self.shuffle:
            ...
            g.manual_seed(self.epoch)
            indices = torch.randperm(len(self.dataset), generator=g).tolist()
            weights = torch.tensor(self.dataset.weights)[indices]
        else:
            indices = torch.arange(len(self.dataset)).tolist()
        ...
        offset = self.num_samples * self.rank
        indices = indices[offset : offset + self.num_samples]
        weights = weights[offset : offset + self.num_samples]
        assert len(indices) == self.num_samples

        sampled_ids = torch.multinomial(weights, self.num_samples, self.replacement).tolist()
        
        return iter(torch.tensor(indices)[sampled_ids].tolist())
